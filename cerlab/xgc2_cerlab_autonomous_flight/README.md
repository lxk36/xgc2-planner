...
This package depends on [xgc2_cerlab_onboard_detector](https://github.com/Zhefan-Xu/xgc2_cerlab_onboard_detector), [xgc2_cerlab_global_planner](https://github.com/Zhefan-Xu/xgc2_cerlab_global_planner), [xgc2_cerlab_trajectory_planner](https://github.com/Zhefan-Xu/xgc2_cerlab_trajectory_planner), [xgc2_cerlab_map_manager](https://github.com/Zhefan-Xu/xgc2_cerlab_map_manager), [xgc2_cerlab_tracking_controller](https://github.com/Zhefan-Xu/xgc2_cerlab_tracking_controller.git)  and [xgc2_cerlab_time_optimizer](https://github.com/Zhefan-Xu/xgc2_cerlab_time_optimizer.git). Please install those packages first!


```
cd ~/catkin_ws/src
git clone https://github.com/Zhefan-Xu/xgc2_cerlab_autonomous_flight.git
...
To visualize the flight data, we provide the rviz launch files in this [repo](https://github.com/Zhefan-Xu/xgc2_cerlab_remote_control) for convenience.
```
...
```
roslaunch xgc2_cerlab_autonomous_flight takeoff_and_hover.launch  # takeoff at 1.0 meter height (by default parameter)
```
b. Takeoff and tracking a circle ```Takeoff/Tracking Circle```:
```
roslaunch xgc2_cerlab_autonomous_flight takeoff_and_track_circle.launch  # takeoff and tracking a circular trajectory
```
c. Static environment navigation ```Navigation```:
```
roslaunch xgc2_cerlab_autonomous_flight navigation.launch   # navigation in the static enviornments
```
d. Dynamic environment navigation ```Dynamic Navigation```:
```
roslaunch xgc2_cerlab_autonomous_flight dynamic_navigation.launch     # navigation in the dynamic environments
```
...
roslaunch xgc2_cerlab_autonomous_flight inspection.launch
```
f. Dynamic environment inspection using dynamic map ```Inspection (Dynamic Map)```:
```
roslaunch xgc2_cerlab_autonomous_flight dynamic_inspection.launch    # autonomous surface inspection
```
g. Unknown and dynamic environment exploration  ```Dynamic Exploration```:
```
roslaunch xgc2_cerlab_autonomous_flight dynamic_exploration.launch    # autonomous exploration in unknown and dynamic environments
```


## IV. Parameter Adjustment
All the parameters are in ```xgc2_cerlab_autonomous_flight/cfg``` folder. For example, the parameters for Dynamic Navigation can be found in xgc2_cerlab_autonomous_flight/cfg/dynamic_navgation/***.yaml
