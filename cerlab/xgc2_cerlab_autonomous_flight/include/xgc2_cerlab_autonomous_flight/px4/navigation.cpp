...
*/
#include <xgc2_cerlab_autonomous_flight/px4/navigation.h>
...
namespace AutoFlight{
	...
	void navigation::initParam(){
    	// parameters    
		// Use global Planner
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/use_global_planner", this->useGlobalPlanner_)){
		...
		// No turning of yaw
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/no_yaw_turning", this->noYawTurning_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/use_yaw_control", this->useYawControl_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/trajectory_info_save_path", this->trajSavePath_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/use_time_optimizer", this->useTimeOptimizer_)){
	...
	void navigation::run(){
		...
		int temp3 = system("rosbag record -O ~/rosbag_navigation_info/navigation_info.bag /camera/color/image_raw /occupancy_map/inflated_voxel_map /navigation/bspline_trajectory /mavros/local_position/pose /mavros/setpoint_position/local /xgc2_cerlab_tracking_controller/vel_and_acc_info /xgc2_cerlab_tracking_controller/target_pose /xgc2_cerlab_tracking_controller/trajectory_history /trajDivider/braking_zone /trajDivider/kdtree_range __name:=navigation_bag_info &");
