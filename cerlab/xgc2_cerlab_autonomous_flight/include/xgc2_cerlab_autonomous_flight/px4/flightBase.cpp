...
*/
#include <xgc2_cerlab_autonomous_flight/px4/flightBase.h>
namespace AutoFlight{
	flightBase::flightBase(const ros::NodeHandle& nh) : nh_(nh){
    	// parameters    	
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/takeoff_height", this->takeoffHgt_)){
		...
		this->statePub_ = this->nh_.advertise<xgc2_cerlab_tracking_controller::Target>("/xgc2_cerlab_autonomous_flight/target_state", 1000);
	...
	void flightBase::circle(){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/circle_radius", this->radius_)){
			this->radius_ = 2.0;
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/time_to_max_radius", this->timeStep_)){
			this->timeStep_ = 30;
        ...
        if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/yaw_control", this->yawControl_)){
        ...
        if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/velocity", this->velocity_)){
			this->velocity_ = 0.5;
	...
	void flightBase::run(){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/radius", r)){
			r = 2.0;
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/circle_velocity", v)){
			v = 1.0;
