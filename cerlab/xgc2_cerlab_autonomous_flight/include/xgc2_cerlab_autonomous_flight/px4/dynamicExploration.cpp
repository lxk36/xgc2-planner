...
#include <xgc2_cerlab_autonomous_flight/px4/dynamicExploration.h>
...
namespace AutoFlight{
	...
	void dynamicExploration::initParam(){
		...
		//  desired angular velocity
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/waypoint_stablize_time", this->wpStablizeTime_)){
		...
		//  initial scan
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/initial_scan", this->initialScan_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/free_range", freeRangeTemp)){
    	...
    	// reach goal distance
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/reach_goal_distance", this->reachGoalDistance_)){
	...
	void dynamicExploration::run(){
		...
		int temp3 = system("rosbag record -O ~/rosbag_exploration_info/exploration_info.bag /camera/aligned_depth_to_color/image_raw_t /camera/color/image_raw_t /dynamic_map/inflated_voxel_map_t /xgc2_cerlab_onboard_detector/dynamic_bboxes /mavros/local_position/pose /dynamicExploration/bspline_trajectory /mavros/setpoint_position/local /xgc2_cerlab_tracking_controller/target_pose /dep/best_paths /dep/roadmap /dep/candidate_paths /dep/best_paths /dep/frontier_regions /dynamic_map/2D_occupancy_map __name:=exploration_bag_info &");
