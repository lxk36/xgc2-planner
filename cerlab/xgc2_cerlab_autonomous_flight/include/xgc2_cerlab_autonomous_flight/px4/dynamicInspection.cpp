...
#include <xgc2_cerlab_autonomous_flight/px4/dynamicInspection.h>
...
namespace AutoFlight{
	...
	void dynamicInspection::initParam(){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/use_fake_detector", this->useFakeDetector_)){
		...
		// desired velocity
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/desired_velocity", this->desiredVel_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/desired_acceleration", this->desiredAcc_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/desired_angular_velocity", this->desiredAngularVel_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_velocity", this->inspectionVel_)){
		...
		// minimum wall area
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/min_wall_area", this->minWallArea_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/safe_distance_to_wall", this->safeDistance_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/side_safe_distance", this->sideSafeDistance_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_height", inspectionHeightsTemp)){
		...
		// ascend step
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/ascend_step", this->ascendStep_)){
		...
		// descend step
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/descend_step", this->descendStep_)){
		...
		// sensor range
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/sensor_range", this->sensorRange_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/sensor_angle_horizontal", this->sensorAngleH_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/sensor_angle_vertical", this->sensorAngleV_)){
		...
		// sample number
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/explore_sample_number", this->exploreSampleNum_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_goal_given", this->inspectionGoalGiven_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_width_given", this->inspectionWidthGiven_)){
		...
		if (this->inspectionGoalGiven_){
			...
			if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_goal", inspectionGoalsTemp)){
				this->inspectionGoalGiven_ = false;
		...
		if (this->inspectionWidthGiven_){
			...
			if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_width", inspectionWidthsTemp)){
				this->inspectionWidthGiven_ = false;
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/zigzag_inspection", this->zigzagInspection_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/fringe_inspection", this->fringeInspection_)){
		...
		// inspection order
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspect_left_first", this->leftFirst_)){
		...
		// confirm max angle
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/confirm_max_angle", this->confirmMaxAngle_)){
		...
		// inspection confirm
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/inspection_confirm", this->inspectionConfirm_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/backward_no_turn", this->backwardNoTurn_)){
		...
		if (not this->nh_.getParam("xgc2_cerlab_autonomous_flight/replan_time_for_dynamic_obstacles", this->replanTimeForDynamicObstacle_)){
	...
	void dynamicInspection::trajExeCB(const ros::TimerEvent&){
		if (this->flightState_ == FLIGHT_STATE::FORWARD or (this->flightState_ == FLIGHT_STATE::BACKWARD and this->prevState_ != FLIGHT_STATE::INSPECT) or (this->flightState_ == FLIGHT_STATE::EXPLORE and this->prevState_ == FLIGHT_STATE::EXPLORE)){
			if (this->trajectoryReady_){
				...
				xgc2_cerlab_tracking_controller::Target target;
