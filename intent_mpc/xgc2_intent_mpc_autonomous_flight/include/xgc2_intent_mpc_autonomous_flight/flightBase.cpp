...
*/
#include <xgc2_intent_mpc_autonomous_flight/flightBase.h>

namespace AutoFlight{
	flightBase::flightBase(const ros::NodeHandle& nh) : nh_(nh){   
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/simulation", this->simulation_)){
    	...
    	// parameters    	
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/takeoff_height", this->takeoffHgt_)){
		...
		if (this->simulation_){
			...
			this->statePub_ = this->nh_.advertise<xgc2_cerlab_tracking_controller::Target>("/xgc2_intent_mpc_autonomous_flight/target_state", 1000);
	...
	void flightBase::takeoff(){
		...
		// xgc2_cerlab_tracking_controller::Target psT;
	...
	void flightBase::circle(){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/circle_radius", this->radius_)){
			this->radius_ = 2.0;
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/time_to_max_radius", this->timeStep_)){
			this->timeStep_ = 30;
        ...
        if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/yaw_control", this->yawControl_)){
        ...
        if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/velocity", this->velocity_)){
			this->velocity_ = 0.5;
	...
	void flightBase::run(){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/circle_radius", r)){
			r = 2.0;
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/velocity", v)){
			v = 1.0;
	...
	void flightBase::updateTargetWithState(const xgc2_cerlab_tracking_controller::Target& target){
