...
namespace AutoFlight{
	...
	void mpcNavigation::initParam(){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/use_fake_detector", this->useFakeDetector_)){
		...
		// use predictor	
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/use_predictor", this->usePredictor_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/use_global_planner", this->useGlobalPlanner_)){
		...
		// No turning of yaw
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/no_yaw_turning", this->noYawTurning_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/use_yaw_control", this->useYawControl_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/desired_velocity", this->desiredVel_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/desired_acceleration", this->desiredAcc_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/desired_angular_velocity", this->desiredAngularVel_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/use_predefined_goal", this->usePredefinedGoal_)){
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/predefined_goal_directory", this->refTrajPath_)){
			this->refTrajPath_ = "None";
			cout << "[AutoFlight]: No predefined goal directory directory." << endl;
		} 
		else{
			std::string autoFlightPkgPath = ros::package::getPath("xgc2_intent_mpc_autonomous_flight");
		...
		if (not this->nh_.getParam("xgc2_intent_mpc_autonomous_flight/execute_path_times", this->repeatPathNum_)){
