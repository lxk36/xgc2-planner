...
mader_common_pkg="ros-noetic-xgc2-mader-common"
...
case "${PACKAGE_GROUP}" in
  ...
  mader-common)
    build_ros_group_deb \
      ...
      "XGC2 shared dependencies for MADER planners" \
      behavior_selector cgal_wrapper decomp_ros_msgs decomp_ros_utils decomp_util eigen_stl_containers graph_msgs rviz_visual_tools separator snapstack_msgs view_controller_msgs
    ;;
  mader)
    build_ros_group_deb \
      "${mader_pkg}" \
      "${mader_common_pkg} (= ${VERSION}), libnlopt0, ros-noetic-gazebo-msgs, ros-noetic-jsk-rviz-plugins, ros-noetic-roscpp, ros-noetic-rospy, ros-noetic-sensor-msgs" \
  ...
  robust-mader)
    build_ros_group_deb \
      "${rmader_pkg}" \
      "${mader_common_pkg} (= ${VERSION}), libcgal-dev, libnlopt0, ros-noetic-jsk-rviz-plugins, ros-noetic-roscpp, ros-noetic-rospy, ros-noetic-sensor-msgs" \
  ...
  meta)
    build_meta_deb \
      "${all_meta_pkg}" \
      "${gcopter_pkg} (= ${VERSION}), ${mader_common_pkg} (= ${VERSION}), ${mader_pkg} (= ${VERSION}), ${rmader_pkg} (= ${VERSION})" \
